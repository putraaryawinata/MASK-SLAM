     ...
     mpCamera(pCamera),mpCamera2(nullptr)
{
    ...
    else // if there was no imGrayPre, like when the initializing
    {
        std::swap(imGrayPre, imGrayT);
    }
    // cout << "T_M: " << T_M << endl;

    // Remove keypoint inside the mask
    int M = mvKeys.size();
    // cout << "mvKeys " << endl;
    ...
    if (M<9000 && M!=0){
        ...
        for (int i =0; i< M; ++i){
            ...
            cv::KeyPoint target_keypoint(x_r, y_r, 1.0f);
            // cout << x_r << ", " << y_r << ";" << endl;
...
void Frame::ProcessMovingObject(const cv::Mat &imgray)
{
    ...
    cv::cornerSubPix(imGrayPre, prepoint, cv::Size(10, 10), cv::Size(-1, -1), cv::TermCriteria(1 | 2, 20, 0.03));
	cv::calcOpticalFlowPyrLK(imGrayPre, imgray, prepoint, nextpoint, state, err, cv::Size(22, 22), 5, cv::TermCriteria(1 | 2, 20, 0.01));
